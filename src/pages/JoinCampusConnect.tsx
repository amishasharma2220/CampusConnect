...
import { ArrowLeft, Eye, EyeOff, Mail, Lock, User, Hash, Sparkles, GraduationCap, Shield, Calendar, Users, Award } from "lucide-react";
import { Link, useNavigate } from "react-router-dom";
import { useToast } from "@/hooks/use-toast";

type Role = "student" | "club_admin";

const JoinCampusConnect = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(selectedRole === "club_admin" ? "/club/dashboard" : "/student/dashboard");
  ...
  const benefits = [
    { icon: <Calendar className="w-5 h-5 text-primary" />, title: "Discover Events", desc: "Browse 150+ events across tech, cultural, sports & academics" },
    { icon: <Users className="w-5 h-5 text-primary" />, title: "Join Clubs", desc: "Register for 50+ student clubs across all faculties" },
    { icon: <Award className="w-5 h-5 text-primary" />, title: "Earn Certificates", desc: "Get digital certificates for events you attend" },
    { icon: <Shield className="w-5 h-5 text-primary" />, title: "Host Events", desc: "Organize and manage events as a club admin" },
  ];
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div className="max-w-md space-y-8">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
              ...
              <h1 className="font-display text-4xl xl:text-5xl font-bold text-secondary-foreground leading-tight mb-4">
                Join <span className="text-gradient">CampusConnect</span>
              </h1>
              <p className="text-secondary-foreground/60 text-lg leading-relaxed mb-8">
                Your all-in-one platform for campus events, clubs, certificates, and student life at MUJ.
              </p>
            </motion.div>

            <div className="space-y-4">
              {benefits.map((b, i) => (
                <motion.div key={b.title} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + i * 0.1 }}
                  className="flex items-start gap-3 p-3 rounded-xl bg-secondary-foreground/5 border border-secondary-foreground/10"
                >
                  <div className="w-10 h-10 rounded-lg bg-primary/15 flex items-center justify-center shrink-0">{b.icon}</div>
                  <div>
                    <p className="font-semibold text-secondary-foreground text-sm">{b.title}</p>
                    <p className="text-secondary-foreground/50 text-xs">{b.desc}</p>
                  </div>
                </motion.div>
              ))}
            </div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md py-8">
          <div className="lg:hidden mb-10">
            <Link to="/" className="flex items-center gap-2">
              ...
              <span className="font-display font-bold text-xl text-foreground">Campus<span className="text-gradient">Connect</span></span>
            </Link>
          </div>
          ...
          <p className="text-muted-foreground mb-8">Join thousands of MUJ students on CampusConnect</p>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button type="button" onClick={() => setSelectedRole("student")}
                className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all ${selectedRole === "student" ? "border-primary bg-primary/5" : "border-border bg-card hover:border-primary/30"}`}
              >
                <GraduationCap className={`w-5 h-5 ${selectedRole === "student" ? "text-primary" : "text-muted-foreground"}`} />
                <div className="text-left">
                  <p className={`text-sm font-semibold ${selectedRole === "student" ? "text-primary" : "text-foreground"}`}>Explore & Join</p>
                  <p className="text-xs text-muted-foreground">Events, clubs, certificates</p>
                </div>
              </button>
              <button type="button" onClick={() => setSelectedRole("club_admin")}
                className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all ${selectedRole === "club_admin" ? "border-primary bg-primary/5" : "border-border bg-card hover:border-primary/30"}`}
              >
                <Shield className={`w-5 h-5 ${selectedRole === "club_admin" ? "text-primary" : "text-muted-foreground"}`} />
                <div className="text-left">
                  <p className={`text-sm font-semibold ${selectedRole === "club_admin" ? "text-primary" : "text-foreground"}`}>Organize Events</p>
                  <p className="text-xs text-muted-foreground">As a club admin</p>
                </div>
              </button>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="button" onClick={(e) => handleSubmit(e as any)} variant="hero" size="lg" className="w-full h-12 rounded-xl text-base mt-2" disabled={isLoading}>
              {isLoading ? (
...
export default JoinCampusConnect;
